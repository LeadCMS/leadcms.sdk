...
### 🔐 Device Authentication Flow (OAuth-like)
...
🔍 Checking LeadCMS version...
   Version: 1.2.88-pre

🔐 Starting device authentication...

📋 To complete authentication, open this link in your browser:
...
### 🚀 Integrated Authentication in Init Flow
...
Enter your LeadCMS URL [http://localhost:45437]: ← Press Enter to keep existing
...
**Benefits:**
- ✅ Single command setup (no separate `leadcms login` needed)
- ✅ Clear explanation of authentication benefits
- ✅ Optional - can skip for read-only access
- ✅ Graceful error handling with fallback to read-only mode
...
**Improvements:**
- **Existing Value Detection** - Reads from `.env` and `leadcms.config.json`
...
⚠️  Configuration files already exist. Overwrite? (y/N): y

Enter your LeadCMS URL [https://my-instance.leadcms.ai]: ← Press Enter
Default language code [en-US]: ← Press Enter
Content directory [.leadcms/content]: ← Press Enter
```
...
Default language code [en-US]: ← Press Enter to accept or type another code
...
### 💬 Comments Support & Selective Pulling
...
Extracted authentication logic into a reusable internal module:

**Internal Functions (CLI use only):**
...
- **CLI-only access** - Use `npx leadcms login` instead of programmatic auth
...
**Improved Messaging:**
- ✅ Clear authentication explanations
- ✅ Contextual prompts (only show "No API key" if `.env` exists)
- ✅ Helpful next steps after configuration
- ✅ Graceful error handling with recovery options

**Enhanced CLI Output:**
- 🔐 Authentication Setup section with clear benefits
- 📋 Visual language list with default markers
- ✓ Success confirmations for each step
- ⚠️ Clear error messages with recovery instructions
...
### Added
- `src/lib/auth.ts` - Shared authentication library (internal use only)
...
- Removed programmatic auth API from public exports
