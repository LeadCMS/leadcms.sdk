...
function maskHeaders(headers: Record<string, unknown>): Record<string, string> {
  ...
  for (const [key, val] of Object.entries(headers)) {
    ...
    if (key.toLowerCase() === "authorization") {
      ...
      safe[key] = parts.length === 2 ? `${parts[0]} ${parts[1].substring(0, 8)}…` : "***";
...
function formatPayload(data: unknown): string {
  ...
  if (
    typeof data === "object" &&
    typeof (data as Record<string, unknown>).getHeaders === "function"
  ) {
...
export function registerApiLogger(): void {
  ...
  axios.interceptors.request.use(
    (config: InternalAxiosRequestConfig) => {
      ...
      (config as unknown as Record<string, unknown>).__logKey = key;
  ...
  axios.interceptors.response.use(
    (response: AxiosResponse) => {
      ...
      const key = (response.config as unknown as Record<string, unknown>).__logKey as
        | string
        | undefined;
      let duration = "";
    ...
    (error: AxiosError) => {
      if (isVerbose()) {
        const key = (error.config as unknown as Record<string, unknown> | undefined)?.__logKey as
          | string
          | undefined;
        let duration = "";
        if (key && pendingRequests.has(key)) {
        ...
        if (error.response?.data) {
          parts.push(`   body:`);
          parts.push(
            formatPayload(error.response.data)
              .split("\n")
              .map((l) => `   ${l.trimStart()}`)
              .join("\n")
          );
