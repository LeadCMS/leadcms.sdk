  ...
  describe("URL Transformation for Push (Backward Transformation)", () => {
    it("should transform local media URLs back to API media URLs when formatting for API", async () => {
      ...
      const localContent = {
        ...
        gallery: ["/media/gallery/image1.jpg", "/media/gallery/image2.png"],
        body: `---
title: Test Article
coverImageUrl: /media/blog/covers/test-article.jpg
---
...
<Image src="/media/gallery/hero.webp" alt="Hero" />`,
      ...
      expect(result.metadata.heroSection.ctaButton.backgroundImage).toBe(
        "/api/media/buttons/cta.png"
      );
    ...
    it("should preserve non-media URLs during backward transformation", async () => {
      ...
      const contentWithMixedUrls = {
        ...
        description:
          "Visit https://example.com/media/external.jpg for external media, or check /media/local.jpg for local media",
      };
      ...
      expect(result.description).toBe(
        "Visit https://example.com/media/external.jpg for external media, or check /api/media/local.jpg for local media"
      );
    });
        ...
        sections: [
          {
            ...
            items: [{ icon: "/media/icons/icon1.svg" }, { icon: "/media/icons/icon2.svg" }],
          },
        ],
  ...
  describe("updateLocalMetadata - updatedAt Sync After Push", () => {
    ...
    it("should update updatedAt in local MDX file after successful push", async () => {
      ...
      const { updateLocalMetadata } =
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (await import("../src/scripts/push-leadcms-content.js")) as any;
  ...
  describe("status mode with missing content types", () => {
    ...
    async function runStatusOnlyMissingTypesTest(apiKey?: string) {
      ...
      expect(consoleError).toHaveBeenCalledWith(
        expect.stringContaining("Missing content types in remote LeadCMS")
      );
      expect(consoleLog).toHaveBeenCalledWith(
        expect.stringContaining("Status mode only reports missing content types")
      );
