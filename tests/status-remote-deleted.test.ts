...
describe("buildSegmentStatus remote-deleted detection", () => {
  ...
  it("reports remote-deleted when local segment has an ID not present on remote", async () => {
    ...
    await fs.writeFile(path.join(tmpDir, "segment-a.json"), JSON.stringify(segment), "utf8");
  ...
  it("reports create (not remote-deleted) when local segment has no ID", async () => {
    ...
    await fs.writeFile(path.join(tmpDir, "new-segment.json"), JSON.stringify(segment), "utf8");
  ...
  it("reports create for remote-only segment when showDelete is false", async () => {
    mockGetAllSegments.mockResolvedValue([
      {
        id: 9,
        name: "Remote Segment",
    ...
    const result = await buildSegmentStatus({ showDelete: false });

    expect(result.operations).toHaveLength(1);
    expect(result.operations[0].type).toBe("create");
    expect(result.operations[0].local).toBeUndefined();
    expect(result.operations[0].remote?.name).toBe("Remote Segment");
    expect(result.operations[0].reason).toBe("New segment on remote");
  });
  ...
  it("reports update (not remote-deleted) when local segment ID still exists on remote", async () => {
    ...
    await fs.writeFile(path.join(tmpDir, "existing-segment.json"), JSON.stringify(segment), "utf8");
...
describe("buildSequenceStatus remote-deleted detection", () => {
  ...
  it("reports create (not remote-deleted) when local sequence has no ID", async () => {
    ...
    await fs.writeFile(path.join(tmpDir, "new-sequence.json"), JSON.stringify(sequence), "utf8");
  ...
  it("reports create for remote-only sequence when showDelete is false", async () => {
    mockGetAllSequences.mockResolvedValue([
      {
        id: 10,
    ...
    const result = await buildSequenceStatus({ showDelete: false });

    expect(result.operations).toHaveLength(1);
    expect(result.operations[0].type).toBe("create");
    expect(result.operations[0].local).toBeUndefined();
    expect(result.operations[0].remote?.name).toBe("Remote Sequence");
    expect(result.operations[0].reason).toBe("New sequence on remote");
  });
  ...
  it("does not report remote-deleted when local sequence ID still exists on remote", async () => {
    ...
    await fs.writeFile(path.join(tmpDir, "active-sequence.json"), JSON.stringify(sequence), "utf8");
