...
describe("Localization - getAllContentSlugsForLocale", () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(contentDir, "si", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\nHome content`
    );
    fs.writeFileSync(
      path.join(contentDir, "si", "contact-us.mdx"),
      `---\nslug: contact-us\ntype: page\npublishedAt: ${publishedDate}\n---\nContact content`
    );
    fs.writeFileSync(
      path.join(contentDir, "si", "contact-us-form.json"),
      JSON.stringify({ slug: "contact-us-form", type: "form", publishedAt: publishedDate })
    );
    fs.writeFileSync(
      path.join(contentDir, "si", "not-found.mdx"),
      `---\nslug: not-found\ntype: page\npublishedAt: ${publishedDate}\n---\nNot found`
    );
    fs.writeFileSync(
      path.join(contentDir, "si", "legal", "privacy.mdx"),
      `---\nslug: legal/privacy\ntype: page\npublishedAt: ${publishedDate}\n---\nPrivacy`
    );
    fs.writeFileSync(
      path.join(contentDir, "si", "legal", "terms.mdx"),
      `---\nslug: legal/terms\ntype: page\npublishedAt: ${publishedDate}\n---\nTerms`
    );

    // Create default language content in root directory (for 'en' locale test)
    fs.writeFileSync(
      path.join(contentDir, "about.mdx"),
      `---\nslug: about\ntype: page\npublishedAt: ${publishedDate}\n---\nAbout page`
    );
    fs.writeFileSync(
      path.join(contentDir, "services.json"),
      JSON.stringify({ slug: "services", type: "page", publishedAt: publishedDate })
    );
  ...
  it("should return slugs for Russian locale ru (another failing case)", () => {
    ...
    fs.writeFileSync(
      path.join(contentDir, "ru", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\nДомашняя страница`
    );
    fs.writeFileSync(
      path.join(contentDir, "ru", "about.json"),
      JSON.stringify({ slug: "about", type: "page", publishedAt: publishedDate })
    );
  ...
  it("should return slugs for US English locale en-US (extended locale format)", () => {
    ...
    fs.writeFileSync(
      path.join(contentDir, "en-US", "products.mdx"),
      `---\nslug: products\ntype: page\npublishedAt: ${publishedDate}\n---\nProducts page`
    );
    fs.writeFileSync(
      path.join(contentDir, "en-US", "services.json"),
      JSON.stringify({ slug: "services", type: "page", publishedAt: publishedDate })
    );
  ...
  it("should return slugs for Russian Russia locale ru-RU (extended locale format)", () => {
    ...
    fs.writeFileSync(
      path.join(contentDir, "ru-RU", "index.mdx"),
      `---\nslug: index\ntype: page\npublishedAt: ${publishedDate}\n---\nГлавная страница`
    );
    fs.writeFileSync(
      path.join(contentDir, "ru-RU", "category", "tech.mdx"),
      `---\nslug: category/tech\ntype: page\npublishedAt: ${publishedDate}\n---\nТехнологии`
    );
    fs.writeFileSync(
      path.join(contentDir, "ru-RU", "news.json"),
      JSON.stringify({ slug: "news", type: "page", publishedAt: publishedDate })
    );
  ...
  it("should return slugs for German locale de (short locale format)", () => {
    ...
    fs.writeFileSync(
      path.join(contentDir, "de", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\nStartseite`
    );
    fs.writeFileSync(
      path.join(contentDir, "de", "company", "about.mdx"),
      `---\nslug: company/about\ntype: page\npublishedAt: ${publishedDate}\n---\nÜber uns`
    );
  ...
  it("should return slugs for French Canada locale fr-CA (regional variant)", () => {
    ...
    fs.writeFileSync(
      path.join(contentDir, "fr-CA", "accueil.mdx"),
      `---\nslug: accueil\ntype: page\npublishedAt: ${publishedDate}\n---\nPage d'accueil`
    );
    fs.writeFileSync(
      path.join(contentDir, "fr-CA", "contact.json"),
      JSON.stringify({ slug: "contact", type: "form", publishedAt: publishedDate })
    );
