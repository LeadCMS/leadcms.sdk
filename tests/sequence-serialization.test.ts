...
const _emptyTemplateMap: EmailTemplateIdNameMap = new Map();

function makeTemplateMaps(): {
  idToName: EmailTemplateIdNameMap;
  nameToId: EmailTemplateNameIdMap;
} {
  const idToName: EmailTemplateIdNameMap = new Map([
    [10, "WS_Email_1"],
    [20, "WS_Email_2"],
    [30, "WS_Email_3"],
  ]);
  const nameToId: EmailTemplateNameIdMap = new Map([
    ["WS_Email_1", 10],
    ["WS_Email_2", 20],
    ["WS_Email_3", 30],
  ]);
...
describe("toLocalSequence", () => {
  ...
  it("should preserve allowedWeekDays when non-null", () => {
    const remote = makeRemoteSequence({
      steps: [
        makeRemoteStep({
          timing: { delay: { value: 0, unit: "days" }, allowedWeekDays: ["Monday", "Tuesday"] },
        }),
      ],
    });
    const local = toLocalSequence(remote, emptySegmentMap, idToName);
      ...
      steps: [
        {
          ...
          createdAt: "2026-03-21T18:41:02Z",
          updatedAt: "2026-03-21T18:41:02Z",
        },
        {
          id: 6,
          sequenceId: 2,
  ...
  it("should preserve step order from local array", () => {
    const local: LocalSequenceDto = {
      ...
      steps: [
        ...
        {
          emailTemplateName: "WS_Email_2",
          name: "Second",
          type: "Email",
          timing: { delay: { value: 1, unit: "days" } },
        },
        {
          emailTemplateName: "WS_Email_3",
          name: "Third",
          type: "Email",
          timing: { delay: { value: 2, unit: "days" } },
        },
      ],
    };
