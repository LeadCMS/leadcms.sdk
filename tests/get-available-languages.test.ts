...
describe("getAvailableLanguages - Issue Reproduction and Fix", () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(contentDir, "en-US", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\nHome US`
    );
    fs.writeFileSync(
      path.join(contentDir, "fr-CA", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\nAccueil`
    );
    fs.writeFileSync(
      path.join(contentDir, "zh-CN", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\n首页`
    );

    // Non-locale directories (should be ignored)
    fs.writeFileSync(
      path.join(contentDir, "notlocale", "content.mdx"),
      `---\nslug: content\ntype: page\npublishedAt: ${publishedDate}\n---\nContent`
    );
    fs.writeFileSync(
      path.join(contentDir, "toolongname", "content.mdx"),
      `---\nslug: content\ntype: page\npublishedAt: ${publishedDate}\n---\nContent`
    );
