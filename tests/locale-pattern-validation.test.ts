...
describe("getAvailableLanguages - Edge Cases", () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(contentDir, "zh", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\n首页`
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(contentDir, "zh-Hans", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\n简体中文首页`
    );

    // Add content to invalid directories (should be ignored even with content)
    fs.writeFileSync(
      path.join(contentDir, "EN", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\nHome`
    );
    fs.writeFileSync(
      path.join(contentDir, "en_US", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\nHome`
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(contentDir, "blog", "post.mdx"),
      `---\nslug: post\ntype: blog\npublishedAt: ${publishedDate}\n---\nPost`
    );
