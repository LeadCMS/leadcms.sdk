...
describe("pull-content status filters", () => {
  ...
  it("returns only remote-backed operations for pull and skips local-only new files", () => {
    const operations = {
      ...
      rename: [
        {
          local: makeLocal("renamed", 11),
          remote: makeRemote(11, "old-slug"),
          oldSlug: "old-slug",
        },
      ],
      typeChange: [
        {
          local: makeLocal("type-changed", 12),
          remote: makeRemote(12, "type-changed"),
          oldType: "page",
          newType: "article",
        },
      ],
      conflict: [
        {
          local: makeLocal("conflicted", 13),
          remote: makeRemote(13, "conflicted"),
