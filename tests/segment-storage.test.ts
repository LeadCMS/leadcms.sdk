...
import type { SegmentDetailsDto } from "../src/lib/automation-types.js";
...
describe("segment local file format", () => {
  ...
  it("toLocalSegment strips runtime fields and returns flat object", async () => {
    ...
    const local = toLocalSegment(remote as unknown as SegmentDetailsDto);
  ...
  it("buildSegmentIdIndex reads legacy _entityType wrapper files", async () => {
    ...
    const legacy = {
      _entityType: "segment",
      data: { id: 9, name: "Legacy Segment", type: "Dynamic" },
    };
  ...
  it("saveSegmentFile produces flat JSON without _entityType wrapper", async () => {
    ...
    const { content } = saveSegmentFile(segment as unknown as SegmentDetailsDto);
