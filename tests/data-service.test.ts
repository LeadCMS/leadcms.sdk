...
describe("LeadCMS Data Service", () => {
  ...
  describe("Mock Mode", () => {
    ...
    it("should return mock content for getAllContent", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining("[MOCK] Returning mock content data")
      );
    });
    ...
    it("should return mock content types for getContentTypes", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining("[MOCK] Returning mock content types")
      );
    });
    ...
    it("should create content in mock mode", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining("[MOCK] Creating content: Test Post")
      );
    });
    ...
    it("should update content in mock mode", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(`[MOCK] Updating content ID ${newContent.id}: Updated Post`)
      );
    });
    ...
    it("should create content type in mock mode", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining("[MOCK] Creating content type: news")
      );
    });
    ...
    it("should switch mock scenarios", () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining("[MOCK] Switched to scenario: Has Conflicts")
      );
    });
  ...
  describe("API Mode", () => {
    ...
    it("should fetch content types from API", async () => {
      const mockContentTypes = [{ uid: "article", format: "MDX", name: "Article" }];
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith("https://api.example.com/api/content-types", {
        headers: {
          Authorization: "Bearer test-api-key",
          "Content-Type": "application/json",
        },
    ...
    it("should return empty array when content types response is a non-array object", async () => {
      ...
      mockedAxios.get.mockResolvedValue({
        data: { items: [{ uid: "article", format: "MDX", name: "Article" }] },
      });
    ...
    it("should handle 401 authentication error on create", async () => {
      ...
      await expect(
        service.createContent({ slug: "test", title: "Test", type: "article" })
    ...
    it("should handle 422 validation error on create", async () => {
      ...
      const thrownError = await service
        .createContent({ slug: "test", type: "article" })
        .catch((e) => e);
    ...
    it("should handle 401 authentication error on update", async () => {
      ...
      await expect(service.updateContent(1, { title: "Updated" })).rejects.toThrow(
    ...
    it("should throw error when trying to switch scenario in API mode", async () => {
      ...
      expect(() => service.switchMockScenario("test")).toThrow(
        "Cannot switch scenario: not in mock mode"
      );
    });
    ...
    it("should fetch user identity from /api/users/me", async () => {
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith("https://api.example.com/api/users/me", {
  ...
  describe("Sequence API", () => {
    ...
    it("should include filter[include]=steps when fetching sequences", async () => {
      const mockSequences = [{ id: 1, name: "Onboarding", steps: [{ id: 10, name: "Step 1" }] }];
  ...
  describe("Email Template API Key Guards", () => {
    ...
    it("should throw for createEmailGroup without API key", async () => {
      const service = new LeadCMSDataService();
      await expect(service.createEmailGroup({ name: "Test", language: "en" })).rejects.toThrow(
    ...
    it("should throw for createEmailTemplate without API key", async () => {
      const service = new LeadCMSDataService();
      await expect(service.createEmailTemplate({ name: "Test" })).rejects.toThrow(
    ...
    it("should throw for updateEmailTemplate without API key", async () => {
      const service = new LeadCMSDataService();
      await expect(service.updateEmailTemplate(1, { name: "Updated" })).rejects.toThrow(
        "Email template operations require authentication. Please configure LEADCMS_API_KEY."
      );
    ...
    it("should throw for deleteEmailTemplate without API key", async () => {
      const service = new LeadCMSDataService();
      await expect(service.deleteEmailTemplate(1)).rejects.toThrow(
        "Email template operations require authentication. Please configure LEADCMS_API_KEY."
      );
  ...
  describe("Comment API Key Guards", () => {
    ...
    it("should throw for updateComment without API key", async () => {
      const service = new LeadCMSDataService();
      await expect(service.updateComment(42, { body: "Updated reply" })).rejects.toThrow(
        "Comment operations require authentication. Please configure LEADCMS_API_KEY."
      );
      expect(mockedAxios.patch).not.toHaveBeenCalled();
    });
    ...
    it("should throw for deleteComment without API key", async () => {
      const service = new LeadCMSDataService();
      await expect(service.deleteComment(42)).rejects.toThrow(
        "Comment operations require authentication. Please configure LEADCMS_API_KEY."
      );
      expect(mockedAxios.delete).not.toHaveBeenCalled();
    });
