...
describe("executeMediaPush: post-upload reconciliation", () => {
  it("should rename local file after upload when server lowercases the name", async () => {
    const originalPath = await createLocalFile("img/cases/Neural-Network.webp", "image-content");
    ...
    const operations: MediaOperation[] = [{ type: "create", local, reason: "New file" }];
  ...
  it("should handle reconciliation failure gracefully (still counts as successful upload)", async () => {
    ...
    expect(
      mockLogWarn.mock.calls.some(
        ...
        (c: any[]) => typeof c[0] === "string" && c[0].includes("reconcil")
      )
    ).toBe(true);
  });
});
