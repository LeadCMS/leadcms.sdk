...
import {
  findAndDeleteContentFile,
  buildContentIdIndex,
  deleteContentFilesById,
  extractContentId,
} from "../src/scripts/pull-leadcms-content";
...
Content`
  ...
  it("should handle empty directory", async () => {
    await expect(findAndDeleteContentFile(tmpDir, "1")).resolves.toBeUndefined();
  });
...
describe("buildContentIdIndex", () => {
  ...
  it("should index JSON files by their id", async () => {
    await fs.writeFile(
      path.join(tmpDir, "widget.json"),
      JSON.stringify({ id: 99, title: "Widget" }, null, 2)
    );
