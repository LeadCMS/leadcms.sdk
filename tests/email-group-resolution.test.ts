...
const mockCreateEmailGroup = jest.fn();
...
function makeLocalTemplate(
  overrides: Partial<LocalEmailTemplateItem> = {}
...
describe("buildGroupIndex", () => {
  ...
  it("skips groups with empty names", () => {
    const groups: EmailGroupItem[] = [makeGroup({ id: 1, name: "", language: "en" })];
...
describe("resolveEmailGroupId", () => {
  ...
  it("resolves group by groupName in metadata", () => {
    ...
    const index = buildGroupIndex([makeGroup({ id: 10, name: "Notifications", language: "en" })]);
  ...
  it("resolves Cyrillic group by groupName in metadata", () => {
    ...
    const index = buildGroupIndex([makeGroup({ id: 15, name: "По запросу", language: "ru-RU" })]);
  ...
  it("resolves group by folder name fallback", () => {
    ...
    const index = buildGroupIndex([makeGroup({ id: 20, name: "По запросу", language: "ru-RU" })]);
  ...
  it("templates can resolve emailGroupId after groups are created", async () => {
    ...
    const templates: LocalEmailTemplateItem[] = [
      ...
      makeLocalTemplate({
        ...
        metadata: {
          name: "WS_Onboarding",
          groupName: "Онбординг при регистрации",
          language: "ru-RU",
        },
      }),
    ];
