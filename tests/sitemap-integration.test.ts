...
describe("Sitemap Integration - getAvailableLanguages", () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(contentDir, "en-US", "about.mdx"),
      `---\nslug: about\ntype: page\npublishedAt: ${publishedDate}\n---\nAbout US`
    );
    ...
    fs.writeFileSync(
      path.join(contentDir, "fr-CA", "about.mdx"),
      `---\nslug: about\ntype: page\npublishedAt: ${publishedDate}\n---\nÀ propos`
    );

    // Spanish content
    fs.writeFileSync(
      path.join(contentDir, "es", "home.mdx"),
      `---\nslug: home\ntype: page\npublishedAt: ${publishedDate}\n---\nInicio`
    );
    fs.writeFileSync(
      path.join(contentDir, "es", "about.mdx"),
      `---\nslug: about\ntype: page\npublishedAt: ${publishedDate}\n---\nAcerca de`
    );
  ...
  it("should support the original sitemap use case with multiple locale formats", () => {
    ...
    expect(defaultLangEntries.map((p) => p.slug)).toEqual(
