...
describe("push-leadcms-content - Pure Functions", () => {
  ...
  describe("isLocaleDirectory", () => {
    it("should recognize two-letter language codes under CONTENT_DIR", async () => {
      ...
      const result = await (isLocaleDirectory as any)("/tmp/test-content/en", "/tmp/test-content");
    ...
    it("should reject single-letter names", async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const result = await (isLocaleDirectory as any)("/tmp/test-content/x", "/tmp/test-content");
      expect(result).toBe(false);
    });

    it("should reject numeric names", async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const result = await (isLocaleDirectory as any)("/tmp/test-content/123", "/tmp/test-content");
      expect(result).toBe(false);
    });

    it("should handle three-letter language codes (e.g., deu)", async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const result = await (isLocaleDirectory as any)("/tmp/test-content/deu", "/tmp/test-content");
  ...
  describe("parseContentFile", () => {
    ...
    it("should warn when type is missing from metadata", async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('missing a "type" property'));
      logSpy.mockRestore();
      setVerbose(false);
    });
