...
function makeRemote(
  overrides: Partial<RemoteContentItem> & { id: number; slug: string }
): RemoteContentItem {
  return {
    type: "article",
    language: "en",
...
describe("LeadCMS Status Analysis (Real SDK Logic)", () => {
  ...
  describe("matchContent - New Content Detection", () => {
    it("should detect new content that does not exist remotely", async () => {
      ...
      await fs.writeFile(
        filePath,
        matter.stringify("# New Article", { title: "New Article", type: "article" })
      );
    ...
    it("should detect multiple new content items", async () => {
      ...
      await fs.writeFile(
        file1,
        matter.stringify("Content 1", { title: "Article 1", type: "article" })
      );
      ...
      const local = [
        makeLocal({
          ...
          filePath: file1,
          type: "article",
          metadata: { title: "Article 1", type: "article" },
        }),
        makeLocal({
          slug: "page-1",
          filePath: file2,
          type: "page",
          metadata: { title: "Page 1", type: "page" },
        }),
      ];
  ...
  describe("matchContent - Conflict Detection", () => {
    it("should detect conflict when remote updatedAt is newer than local", async () => {
      ...
      await fs.writeFile(
        filePath,
        matter.stringify("Local content", {
    ...
    it("should detect language-specific conflicts independently", async () => {
      ...
      await fs.writeFile(
        fileEn,
        matter.stringify("English content", {
      ...
      await fs.writeFile(
        fileEs,
        matter.stringify("Spanish content", {
  ...
  describe("matchContent - Slug Changes (Renames)", () => {
    ...
    it("should detect slug rename when matched by title", async () => {
      ...
      await fs.writeFile(
        filePath,
        matter.stringify("About content", {
          title: "About Us",
          type: "page",
          updatedAt: "2024-06-15T00:00:00Z",
        })
  ...
  describe("matchContent - Duplicate remote ID prevention", () => {
    it("should not allow two local items to match the same remote item by frontmatter ID", async () => {
      ...
      const totalMatches =
        ops.rename.length + ops.update.length + ops.typeChange.length + ops.conflict.length;
  ...
  describe("matchContent - Deletion Detection with MetadataMap", () => {
    ...
    it("should detect deletions across multiple locales with stale metadata", async () => {
      const local = [makeLocal({ slug: "article-a", locale: "en", metadata: { type: "article" } })];
