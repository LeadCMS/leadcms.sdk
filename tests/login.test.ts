...
describe("Login Flow (Real SDK Auth Functions)", () => {
  ...
  describe("verifyToken", () => {
    it("should call /api/users/me with Bearer token and return user details", async () => {
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith("https://test.leadcms.com/api/users/me", {
    ...
    it("should propagate 401 errors from the API", async () => {
      ...
      await expect(verifyToken("https://test.leadcms.com", "invalid-token")).rejects.toMatchObject({
        response: { status: 401 },
      });
    });
  });
  ...
  describe("authenticate (integration of version check + auth flow)", () => {
    it("should use manual token flow for old server versions", async () => {
      ...
      const mockQuestion = jest.fn((_prompt: string) => Promise.resolve("manual-token"));
    ...
    it("should use manual flow when version check fails", async () => {
      ...
      const mockQuestion = jest.fn((_prompt: string) => Promise.resolve("fallback-token"));
  ...
  describe("manualTokenFlow", () => {
    it("should return the token provided by the user", async () => {
      const mockQuestion = jest.fn((_prompt: string) => Promise.resolve("user-provided-token"));
    ...
    it("should throw if no token is provided", async () => {
      const mockQuestion = jest.fn((_prompt: string) => Promise.resolve(""));

      await expect(manualTokenFlow("https://test.leadcms.com", mockQuestion)).rejects.toThrow(
        "No token provided"
      );
    });
  });
